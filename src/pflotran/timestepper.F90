...
subroutine StepperStepFlowDT(realization,stepper,failure)
  ...
  use Richards_module, only : RichardsScaleSS
  ...
  select case(option%iflowmode)
    ...
    case(RICHARDS_MODE)
      ...
      if (option%root_system) then
        if(associated(stepper%cur_waypoint)) then
          if(stepper%cur_waypoint%update_conditions) then
            call RichardsScaleSS(realization)
          endif
        endif
      endif
    case(MPH_MODE)
