...
function FieldCreate()
  ...
  field%kcomp_loc = 0
  field%ssd_loc = 0
  field%hthreshold_loc = 0
  field%tact_loc = 0
  field%trootsys_loc = 0
...
subroutine FieldDestroy(field)
  ...
  if (field%kcomp_loc/=0) call VecDestroy(field%kcomp_loc,ierr)
  if (field%ssd_loc/=0) call VecDestroy(field%ssd_loc,ierr)
  if (field%hthreshold_loc/=0) call VecDestroy(field%hthreshold_loc,ierr)
  if (field%tact_loc/=0) call VecDestroy(field%tact_loc,ierr)
  if (field%trootsys_loc/=0) call VecDestroy(field%trootsys_loc,ierr)
